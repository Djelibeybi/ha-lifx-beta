...
class Message(object):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__}, seq: {format(self.seq_num)}, ack: {format(self.ack_requested)}, resp: {format(self.response_requested)}>"

    def __str__(self):
        return ", ".join(
            [
                f"{self.__class__.__name__}",
                f"pkt={format(self.message_type)}",
                f"seq={format(self.seq_num)}",
                f"ack={format(self.ack_requested)}",
                f"resp={format(self.response_requested)}",
            ]
        )
